#include <stdio.h>
  ...
  void matrix_x_vector(int n, int y[n], int x[n][n], int A[n]); // Performs the calculation
...
void matrix_x_vector(int n, int y[n], int x[n][n], int A[n])
{
  ...
  omp_set_nested(1); //setting the nested parallelism to be on....

  //Parallelisation of the outer loop ... outer loop index considers rows
